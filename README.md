...
- **A floating ball** like Immersive Translate's: drag it anywhere — it **snaps
  ...
  <kbd>Alt</kbd>+<kbd>Shift</kbd>+<kbd>P</kbd> — which on a page Anagram is off
  for analyzes that one page instead, granting nothing. **Click the counter** to open the
...
- **Without a mouse.** The chips stay out of the tab order on purpose (hundreds
  of stops, numbers read out mid-sentence), so the panel is the accessible
  ...
  and its score, <kbd>Esc</kbd> closes it and hands focus back.
...
- **A popup that leads with the page.** It opens with what is happening on the
  ...
  line. The **options page** holds all of it and the rest: per-site rules (with
...
popup's **Analyze this page** — or <kbd>Alt</kbd>+<kbd>Shift</kbd>+<kbd>P</kbd> —
runs a single page with nothing granted at all (see
...
| Scoring with no host permission | `node test/daemon-cors-check.mjs` | 14 checks that the SHIPPING build — which asks for no host at all — can still score. Every other browser suite loads the TEST build, whose required `http://*/*` covers loopback and bypasses CORS, so none of them would notice a daemon that answered no CORS headers at all. This one loads the shipping build with nothing granted and requires: `/health` read as an ordinary cross-origin request, a batch that really scores with the daemon counting its preflight, a daemon sending no CORS headers reported as needing an UPDATE rather than as absent, nothing listening still reported as absent, a daemon behind the extension scoring on while asking to be updated, and a web page unable to read the daemon at all. Chrome and Firefox — the `moz-extension://` origin is a separate answer from the daemon and worth its own proof |
...
it and the popup says so. Both real-daemon suites start a daemon of their **own**
on a port the system picked, and stop only that one; `ANAGRAMD_REUSE=1` borrows
the daemon already running on 8765 (or on `ANAGRAMD_PORT`) and leaves it running,
which also means `test:verify` skips its daemon-down half. `ANAGRAMD_PORT` on a
port that is already busy is an error rather than a quiet reuse — no run of these
suites sends your paragraphs through a daemon it was not told to use.
...
mode of their own: the extension page `entrypoints/reader/` is handed the file's
bytes and shows its **real pages** — a canvas per page drawn lazily by pdf.js and released
...
the content script that runs on every granted page stays small — pdf.js, its 1.2 MB
...
| Daemon request limits + Host / Origin allow-lists + CORS for extension origins only | pydantic, Starlette TrustedHost | `anagramd/serve.py` |
