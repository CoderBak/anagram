...
| Accessibility | `npm run test:a11y` | 82 | **axe-core** (WCAG 2.1 A + AA, best-practice rules on a line of their own) on the popup, options, onboarding and PDF reader pages in **light and dark** — options with two site rules and the add-rule error showing, onboarding with the daemon up and stopped, the reader empty and with a PDF the suite writes itself — and then, scoped to OUR nodes only, on the ball with the panel closed, open with flagged rows, open with both verdict filters, on a pinned chip card, the selection card and the daemon-down notice (axe descends into the open shadow roots; the suite proves it did by naming a node it could only have reached through one). Plus everything axe cannot do, asserted in code: Tab reaches the ball then the counter, Enter opens the panel as a named dialog and hands focus over, Tab walks its controls in DOM order with no positive tabindex, Escape closes it and gives focus back; an accessible name (a real word, not a glyph), a visible `:focus-visible` change and a 24x24 CSS-px hit target for every control; colour contrast computed from the RESOLVED colours for the chip number, card verdict, panel percentages and counter in light and dark — axe cannot always see through a top-layer popover in a shadow root, and a chip mid-`background-color` transition reads as a phantom failure, so the suite waits for `document.getAnimations()` to go quiet before it measures; `prefers-reduced-motion` (nothing of ours animating, the jump pulse a static outline) and forced colours (chips keep a boundary, the verdict dot keeps its colour). It starts from an explicit **baseline** of the debt that existed on the day it was written (29 axe rules + 19 code-level findings, each with its reason) and fails only on what is NOT in it; deliberate exemptions — the chips are `aria-hidden` and unfocusable on purpose — are listed apart. JSON report in the artifacts folder |
