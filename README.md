...
- **Text behind "see more", with its chip where you can see it.** Feeds keep the
  ...
  instead of inside it where nobody would find it. Paragraphs still on screen keep
  their chip in place, and opening the post leaves every chip where it is. Scroll
  containers, carousels, `<details>` and a page-level `overflow:hidden` under an
  open modal are not clipping, and nothing moves for them.
