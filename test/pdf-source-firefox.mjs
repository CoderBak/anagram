...
import {launchFirefox, openExtensionPage, hrefOf} from "./firefox-harness.mjs";
...
const server = http.createServer((req,res) => {
  ...
  res.writeHead(200,{"content-type":"application/pdf", "cache-control":"no-store", "content-length":TEST_PDF.length,
    "set-cookie":"anagram_pdf_source=fixture; SameSite=Lax"});res.end(TEST_PDF);
});
...
try {
  ...
  await page.waitForFunction(() => !!window.PDFViewerApplication?.pdfDocument, {timeout:20000}).catch(async(error)=>{
  ...
  await page.evaluate(()=>document.getElementById("original").click()); await page.waitForFunction((url) => location.href === url,{},source);
  ...
  await page.waitForFunction(() => !!window.PDFViewerApplication?.pdfDocument,{timeout:20000});
  ...
  await forged.waitForSelector("#drop:not([hidden])");
