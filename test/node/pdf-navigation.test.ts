...
async function setup(origins = ["https://*/*", "http://*/*"]) {
  ...
  const listen = vi.spyOn(fakeBrowser.webRequest.onHeadersReceived, "addListener").mockImplementation((f) => { headers = f as never; });
  vi.spyOn(fakeBrowser.permissions, "getAll").mockResolvedValue({origins} as never);
  vi.spyOn(fakeBrowser.permissions.onAdded, "addListener").mockImplementation((f) => { granted = f as never; });
  ...
  const route = createPdfNavigation({open, access, setting}); route.serve(); await ticks();
  const event = (extra = {}) => ({tabId: 7, frameId: 0, url: current, ...extra}) as never;
  return {route, open, access, setting, listen, grant: (added: string[]) => granted({origins: added} as never), before: (url = current) => { current = url; before(event()); },
...
describe("PDF navigation without content scripts", () => {
  it("opens authorized MIME PDFs with no filename extension after commit", async () => {
    const e = await setup(); e.before(); e.headers(); await ticks(); expect(e.open).not.toHaveBeenCalled();
  ...
  it("keeps native PDF when disabled or not persistently authorized", async () => {
    for (const disabled of ["setting", "access"] as const) {
      const e = await setup(); e[disabled].mockResolvedValue(false); e.before(); e.headers(); e.commit(); await ticks(); expect(e.open).not.toHaveBeenCalled();
    }
  });
  it("routes authorized local PDFs without content injection", async () => {
    const e = await setup(); e.before("file:///tmp/book.pdf"); e.commit(); await ticks();
  ...
  it("suppresses every event for one Open original navigation, then allows reload", async () => {
    const e = await setup(); e.route.pass(7, "https://example.test/document"); e.before(); e.headers(); e.commit(); e.complete();
  ...
  it("does not navigate a replacement document after asynchronous authorization", async () => {
    const e = await setup(); let allow!: (v: boolean) => void; e.access.mockImplementation(() => new Promise((r) => { allow = r; }));
...
describe("optional site access", () => {
  it("observes response headers only once an http(s) site is granted", async () => {
    const e = await setup([]); expect(e.listen).not.toHaveBeenCalled();
    e.grant(["file:///*"]); expect(e.listen).not.toHaveBeenCalled();
    e.grant(["https://example.test/*"]); e.grant(["https://*/*"]); expect(e.listen).toHaveBeenCalledTimes(1);
    e.before(); e.headers(); e.commit(); await ticks(); expect(e.open).toHaveBeenCalledWith(7, "https://example.test/document", {auto: true});
  });
  it("observes response headers once when a grant already exists", async () => {
    const e = await setup(); e.grant(["https://example.test/*"]); expect(e.listen).toHaveBeenCalledTimes(1);
  });
});
