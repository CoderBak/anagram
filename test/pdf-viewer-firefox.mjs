...
import {launchFirefox, until} from "./firefox-harness.mjs";
...
try{
  ...
  await until(page,()=>document.querySelectorAll(".textLayer .highlight").length>0);
  await chipped();
  ...
  await until(page,()=>PDFViewerApplication.pdfViewer.currentScale===1.5);
  await click("secondaryToolbarToggleButton");await click("pageRotateCw");
  await until(page,()=>PDFViewerApplication.pdfViewer.pagesRotation===90);
  await page.screenshot({path:join(artifacts,"firefox-viewer.png")}).catch(()=>{});
  await choose("password.pdf",LOCKED_PDF);await until(page,()=>!!document.querySelector("#passwordDialog[open]"));
  ...
  console.log(`PASS Firefox ${firefox.version}: ${offline?"offline ":""}full viewer/File input handler/controllers/search/zoom/rotation/password/engine-down/source hash/no external requests`);
