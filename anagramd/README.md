...
in `../docs/benchmarks/`. Both files stay in the source checkout and are excluded from
release archives. Run the benchmark from `anagramd/` after `uv sync --extra bench`
to install its `peft` and `accelerate` dependencies; checkpoints belong in `../../models/`.
It uses local weights by default. Pass `--online` to allow downloading missing weights.
