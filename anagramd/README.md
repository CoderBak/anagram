...
file sizes and SHA-256 hashes, and stages replacements before committing them. The kit
contains the original weights and FP32/FP16/INT8 ONNX variants (about 4.07 GB). The fastText
language model is downloaded and verified separately. No Hugging Face token is required.
...
The shared measurement budget is 30 seconds; load and warmup time are separate. Reports
include batch-one latency, batch-eight throughput and sampled memory use where supported.
FP32 recommendations do not silently choose a quantized model. Subsequent launches reuse
...
them. Component updates and removal are complete only when their maintenance receipt
