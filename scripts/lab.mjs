...
//   npm run lab -- test [suite…] [--headless]   sync + build + run suites on the lab's screen
...
function exec(command, { env = {}, show = true, detach = false } = {}) {
  const { scale } = labEnv();
  ...
  return docker(["exec", ...(detach ? ["-d"] : []), "-u", LAB_USER, "-w", "/work", ...flags, NAME, "sh", "-c", command], { show });
}
...
/** Copy the sources in; rebuild the image first if the lockfile moved on. Returns true when
...
function sync() {
  ...
  const built = exec("test -f output/chrome-mv3/manifest.json", { show: false }).status === 0;
  ...
  if (needsBuild && exec("npm run -s build 2>&1 | tail -4").status !== 0) die("build failed inside the lab");
}
...
function test() {
  ...
  for (const s of suites) {
    ...
    const t0 = Date.now();
    const r = exec(SUITES[s], { env: { ...(headless ? {} : { HEADED: "1" }), ANAGRAM_ARTIFACTS: `/out/${s}`, CI: "1" } });
    rows.push({ suite: s, ok: r.status === 0, seconds: Math.round((Date.now() - t0) / 100) / 10 });
  }
  console.log(`\n━━ lab summary (${headless ? "headless" : "on the lab's screen"}) ${"━".repeat(30)}`);
  for (const r of rows) console.log(`${r.ok ? "PASS" : "FAIL"}  ${r.suite.padEnd(10)} ${r.seconds}s`);
  mkdirSync(RESULTS, { recursive: true });
  writeFileSync(join(RESULTS, "summary.json"), JSON.stringify({ at: new Date().toISOString(), headless, rows }, null, 1));
